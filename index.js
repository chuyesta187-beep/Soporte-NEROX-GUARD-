...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isButton() && (
        ...
        // DESTRUCCIÓN CON AUDITORÍA COMPLETA PAGINADA Y COMPILADA CORRECATAMENTE
        if (action === 'delete') {
            ...
            try {
                ...
                let transcriptText = `=== AUDITORÍA CRÍTICA: ${channel.name} ===\nCliente ID: ${targetUserId}\nAgente ID: ${ticketData.claimedBy}\n\n`;
                ...
                const attachment = new AttachmentBuilder(buffer, { name: `audit-log-${channel.name}.txt` });

                if (logsChannel && logsChannel.isTextBased()) {
                    await logsChannel.send({ content: `🚨 **Canal destruido e indexado:** \`${channel.name}\` de <@${targetUserId}>.`, files: [attachment] });
                }
            } catch (err) {
                console.error("❌ Fallo en compilación paginada:", err);
            }
        ...
        if (action === 'lock') {
            ...
            const newClaimRow = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId(`delete_ticket_${targetUserId}`).setLabel('❌ Eliminar Canal Definitivamente').setStyle(ButtonStyle.Danger)
            );

            const lockedEmbed = EmbedBuilder.from(interaction.message.embeds[0])
                .setColor('#FF3333')
                .setDescription(`👤 **Cliente:** <@${targetUserId}>\n📌 **Estado:** 🔒 Escritura Bloqueada / Archivado\n👮 **Agente:** ${ticketData.claimedBy !== 'none' ? `<@${ticketData.claimedBy}>` : 'Sin asignar'}`);
            
            await interaction.message.edit({ embeds: [lockedEmbed], components: [newMainRow, newClaimRow] });
            return interaction.editReply(`🔒 Canal archivado. Acceso de escritura removido.`);
        }
