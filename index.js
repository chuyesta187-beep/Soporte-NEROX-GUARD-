...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isButton() && (
        ...
        // 🗑️ DESTRUCCIÓN CON AUDITORÍA COMPLETA PAGINADA CORREGIDA
        if (action === 'delete') {
            ...
            try {
                ...
                if (logsChannel && logsChannel.isTextBased()) {
                    await logsChannel.send({ 
                        content: `🚨 **Ticket destruído y archivado de forma permanente:** \`${channel.name}\` perteneciente a <@${targetUserId}>. Atendido por: <@${ticketData.claimedBy}>`, 
                        files: [attachment] 
                    });
                }
            } catch (err) {
